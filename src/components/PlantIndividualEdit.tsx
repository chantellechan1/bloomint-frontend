...
const takePicture = async (userPlantID: number) => {
  try {
    ...
    await server.UploadPlantImage({
      imageB64: image.base64String,
      userPlantID,
    });
  } catch (err) {
    console.error(err);
...
const PlantIndividualEdit = (props: { setLoading: (val: boolean) => void }) => {
  ...
  const userPlantId = Number(params.plantID);
  ...
  useEffect(() => {
    const apiCalls = async () => {
      try {
        setLoading(true);
        const userPlant = await getSinglePlant(userPlantId);
        console.log("userPlant: ", userPlant);

        const [plantTypeInfo, plantTypesFromAPI, plantImagesFromAPI] =
          await Promise.all([
            getPlantTypeInformation(userPlant.plant_id),
            getAllPlantTypes(),
            getPlantImages(userPlantId),
          ]);
    ...
    apiCalls();
  }, [setLoading, userPlantId]);

  const postUpdatePlant = async () => {
    if (!plant) {
      return;
    } // if plant is undefined, don't try to update it
    try {
      props.setLoading(true);
      await server.UpdatePlant(plant);
      props.setLoading(false);
      navigate(`/plant/${plant.user_plant_id}`);
    } catch (e) {
      console.log(e);
    } finally {
      props.setLoading(false);
  ...
  const deletePlant = async () => {
    try {
      props.setLoading(true);
      await server.DeletePlant({ plantID: userPlantId });
      props.setLoading(false);
    ...
    <React.Fragment>
      ...
      >
        ...
        <DialogActions>
          ...
          <div className="btn btn-outline-danger" onClick={deletePlant}>
            Confirm Delete
          </div>
        </DialogActions>
      </Dialog>
          ...
          <div className="row mb-3" key={uuidv4()}>
            <div className="col text-start">
              ...
              {plantImages && plantImages.length > 0 && (
                <div
                  id="userPlantImageCarousel"
                  className="carousel carousel-dark slide"
                  data-bs-ride="carousel"
                >
                  <div className="carousel-inner">
                    {plantImages?.map(
                      (plantImage: PlantImage, index: number) => {
                        return (
                          <div
                            key={uuidv4()}
                            className={`carousel-item ${
                              index === activePlantImageIdx ? "active" : ""
                            }`}
                          >
                            <img
                  ...
                  <button
                    ...
                    type="button"
                    data-bs-target="#userPlantImageCarousel"
                    data-bs-slide="prev"
                    onClick={() => {
                      if (
                        plantImages &&
                        activePlantImageIdx < plantImages.length - 1
                      ) {
              ...
              <div>
                <i>Temp Range: </i> {plantType?.min_temp}°C to{" "}
                {plantType?.max_temp}°C
              </div>
              <div>
                <i>Sun: </i> {plantType?.sunlight}
              </div>
              <div>
                <i>Water: </i> Every {plantType?.water_frequency} day(s)
              </div>
